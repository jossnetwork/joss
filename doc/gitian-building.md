...
*Setup instructions for a Gitian build of Joss Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Joss
...
- [Building Joss Core](#building-joss-core)
...
After Installation
-------------------
The next step in the guide ijosss logging in as root via SSH.
...
Clone the git repositories for Joss Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/jossnetwork/joss
...
in the Joss Core repository under 'Fetch and create inputs' to install sources which require
...
To build Joss Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Joss Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/joss/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/jossnetwork/joss
...
For example:
```bash
URL=https://github.com/crowning-/joss.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit joss=${COMMIT} --url joss=${URL} ../joss/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit joss=${COMMIT} --url joss=${URL} ../joss/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit joss=${COMMIT} --url joss=${URL} ../joss/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the joss git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../joss/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/joss
SIGPATH=/some/root/path/joss-detached-sigs

./bin/gbuild --url joss=${BTCPATH},signature=${SIGPATH} ../joss/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/joss-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/joss-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/joss-osx-build.assert
```
...
[joss/gitian.sigs](https://github.com/jossnetwork/gitian.sigs/) repository, or if that's not possible to create a pull
request.
