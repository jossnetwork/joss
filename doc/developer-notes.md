...
- [Development guidelines](#development-guidelines)
    - [General Joss Core](#general-joss-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Joss Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/jossnetwork/joss/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/jossd -printtoconsole
```
...
Joss Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Joss Core, where everyone uses
...
pay attention to for reviewers of Joss Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Joss Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Joss Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `joss-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `joss-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Joss Core, the wallet was always
