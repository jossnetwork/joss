...
const char * const BITCOIN_CONF_FILENAME = "joss.conf";
const char * const BITCOIN_PID_FILENAME = "jossd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "jossd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JossCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JossCore
    // Mac: ~/Library/Application Support/JossCore
    // Unix: ~/.josscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JossCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JossCore";
#else
    // Unix
    return pathRet / ".josscore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty joss.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Joss Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Joss Core developers";
    }
