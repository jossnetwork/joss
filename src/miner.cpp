...
void static JossMiner(const CChainParams& chainparams)
{
    LogPrintf("JossMiner -- started\n");
    return;
}

int GenerateJosss(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&JossMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
