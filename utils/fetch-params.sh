...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/JossParams"
else
    PARAMS_DIR="$HOME/.joss-params"
fi
...
DOWNLOAD_URL="https://github.com/jossnetwork/joss/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with JOSS_DISABLE_SOMETHING=1
JOSS_DISABLE_WGET="${JOSS_DISABLE_WGET:-}"
#JOSS_DISABLE_IPFS="${JOSS_DISABLE_IPFS:-}"
JOSS_DISABLE_CURL="${JOSS_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$JOSS_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$JOSS_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$JOSS_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Joss parameters!
...
main() {
    ...
    cat <<EOF
JOSS - fetch-params.sh

This script will fetch the Joss SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Joss SNARK parameters. Note that it is
