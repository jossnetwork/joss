...
PACKAGE_VERSION=$($SRC_PATH/src/jossd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/jossd $DEB_BIN
cp $SRC_PATH/src/joss-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/joss-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/jossd.1 $DEB_MAN
cp $SRC_DOC/man/joss-cli.1 $DEB_MAN
cp $SRC_DOC/man/joss-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/jossd.bash-completion $DEB_CMP/jossd
cp $SRC_PATH/contrib/joss-cli.bash-completion $DEB_CMP/joss-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/jossd.1
gzip --best -n $DEB_MAN/joss-cli.1
gzip --best -n $DEB_MAN/joss-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/jossd $DEB_BIN/joss-cli
