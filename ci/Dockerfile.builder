...
# joss_hash
RUN git clone https://github.com/jossnetwork/joss_hash
RUN cd joss_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} joss
RUN useradd -u ${USER_ID} -g joss -s /bin/bash -m -d /joss joss
...
RUN mkdir /joss-src && \
  ...
  chown $USER_ID:$GROUP_ID /joss-src && \
